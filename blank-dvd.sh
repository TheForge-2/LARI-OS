...
if ! wodim dev="$DEVICE" -checkdrive 2>&1 | grep -q -e "DVD-RW" -e "DVD+RW" -e "CD-RW" ; then
...
echo "[!] WARNING: This will blank $DEVICE in fast mode."
...
echo
echo "[*] Starting real blanking in 3 seconds."
echo "    Last chance to cancel with 'Ctrl' + 'C'."
sleep 3s
echo
echo "[*] Blanking $DEVICE in fast mode..."
sudo wodim blank=fast dev=/dev/sr0
echo
